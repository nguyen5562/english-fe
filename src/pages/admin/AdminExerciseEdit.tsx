...
export default function AdminExerciseEdit() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    Promise.all([exerciseService.getExerciseById(id), courseService.getAllCourse()])
      .then(([exData, coData]) => {
        if (!cancelled) {
          setExercise(exData);
          setCourses(coData);
  ...
  return (
    <Box>
      ...
      <Button
        variant="contained"
        startIcon={<AddIcon />}
        onClick={() => navigate(`/admin/exercises/${buildSlugId(exercise.title ?? "", id)}/sections/new`)}
        sx={{ mb: 2 }}
      >
        Thêm phần
      </Button>
      ...
      {(exercise.sections?.length ?? 0) === 0 ? (
        <Typography color="text.secondary">Chưa có phần nào. Nhấn &quot;Thêm phần&quot; để tạo.</Typography>
      ) : (
        <List>
            ...
            <ListItem key={section._id} sx={{ mb: 1, p: 0 }}>
              <Card sx={{ width: "100%" }}>
                <CardContent sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", "&:last-child": { pb: 2 } }}>
                  ...
                  <Box>
                    <IconButton
                      onClick={() => navigate(`/admin/exercises/${buildSlugId(exercise.title ?? "", id)}/sections/${section._id}`)}
                      sx={{ mr: 0.5 }}
                    >
                      <EditIcon />
                    </IconButton>
